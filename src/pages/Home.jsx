    ...
    <section ref={containerRef} className="relative min-h-screen overflow-hidden bg-white">
      ...
      <div className="grid lg:grid-cols-2 min-h-screen">
        <div className="relative overflow-hidden">
          <motion.img src={hero?.backgroundImages?.[0]?.url || 'https://images.unsplash.com/photo-1580273916550-e323be2ae537?w=1400&q=80'} alt="Pro Auto Body Shop" className="absolute inset-0 w-full h-full object-cover object-center" style={{ scale: imgScale }} loading="eager" />
        ...
        <motion.div className="flex flex-col justify-center px-8 sm:px-12 lg:px-16 py-24 lg:py-0 pt-36" style={{ y: textY }}>
          <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.3 }}>
            ...
            <p className="text-orange-500 text-xs font-semibold uppercase tracking-[0.3em] mb-6" style={{ fontFamily: 'var(--font-sans)' }}>Professional Collision Repair</p>
            <div className="overflow-hidden">
              {['EXPERT', 'BODY', 'REPAIR.'].map((line, i) => (
                <motion.div key={line} initial={{ y: '110%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5 + i * 0.12, ease: [0.16, 1, 0.3, 1] }}>
                  <h1 className={`font-heading leading-[0.88] tracking-tight ${i === 1 ? 'text-orange-500' : 'text-neutral-900'}`} style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)', fontWeight: i === 1 ? 900 : 300 }}>{line}</h1>
                </motion.div>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-4 mt-10">
              <Link to="/contact" className="group inline-flex items-center gap-3 bg-orange-500 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-orange-600 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>{hero?.ctaPrimary || 'Get Free Quote'} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" /></Link>
              <a href="tel:+263773193931" className="group inline-flex items-center gap-3 border border-neutral-200 text-neutral-700 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:border-orange-500 hover:text-orange-500 transition-all" style={{ fontFamily: 'var(--font-sans)' }}><Phone size={16} weight="fill" /> Call Us</a>
            </div>
          </motion.div>
        </motion.div>
      </div>
...
function ProcessStrip() {
  ...
  return (
    <section ref={ref} className="bg-orange-500 py-12 sm:py-16">
    ...
    <section ref={ref} className="bg-neutral-50 py-24 sm:py-32 lg:py-40">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20 text-center">
          ...
          <h2 className="font-heading text-neutral-900 leading-[0.92] font-bold uppercase" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>Our <span className="text-orange-500">Services</span></h2>
        </motion.div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {servicesPreview?.slice(0,6).map((service, i) => { const Icon = iconMap[service.icon] || Wrench; return (
            <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.08*i }}>
              <Link to={`/services#${services?.items?.[i]?.slug || ''}`} className="group block bg-white border border-neutral-100 hover:border-orange-500/30 transition-colors overflow-hidden">
    ...
    <section ref={ref} className="bg-white py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12"><div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
        <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
          <div className="w-12 h-[2px] bg-orange-500 mb-6" />
          <h2 className="font-heading text-neutral-900 leading-[0.95] font-bold uppercase mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>About <span className="text-orange-500">Pro Auto</span></h2>
          <p className="text-neutral-600 text-sm sm:text-base leading-relaxed mb-6 max-w-lg" style={{ fontFamily: 'var(--font-sans)' }}>Pro Auto Body Shop on Craster Road delivers expert collision repair and spray painting. Our certified technicians use modern frame straighteners, computerised colour matching, and downdraft spray booths to achieve factory-grade results.</p>
          <div className="flex gap-10"><div><div className="text-orange-500 font-heading text-3xl font-bold">4.2</div><div className="text-neutral-400 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Rating</div></div><div><div className="text-orange-500 font-heading text-3xl font-bold">17</div><div className="text-neutral-400 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Reviews</div></div></div>
        </motion.div>
        <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
          <img src="https://images.unsplash.com/photo-1549317661-bd32c8ce0571?w=800&q=80" alt="Pro Auto workshop" className="w-full aspect-[4/5] object-cover object-center" loading="lazy" />
          <div className="absolute -top-3 -right-3 w-20 h-20 border-t-2 border-r-2 border-orange-500/30" />
        </motion.div>
      </div></div>
    </section>
  );
  ...
  return (
    <section ref={ref} className="bg-neutral-50 py-24 sm:py-32 lg:py-40">
      <div className="max-w-4xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <Quotes size={48} weight="fill" className="text-orange-500/10 mx-auto mb-8" />
        <AnimatePresence mode="wait">
          <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }}>
            ...
            <div className="flex flex-col items-center gap-3"><div className="w-8 h-[2px] bg-orange-500" /><div className="text-neutral-900 text-sm uppercase tracking-[0.15em] font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>{t.name}</div><div className="flex gap-0.5 mt-1">{[...Array(t.rating||5)].map((_,i)=><Star key={i} size={12} weight="fill" className="text-orange-500" />)}</div></div>
          </motion.div>
        </AnimatePresence>
        <div className="flex items-center justify-center gap-6 mt-12">
          <button onClick={prev} className="w-10 h-10 border border-neutral-200 flex items-center justify-center text-neutral-400 hover:text-orange-500 transition-colors"><CaretLeft size={16} /></button>
          <div className="flex gap-2">{testimonials.map((_,i)=><button key={i} onClick={()=>setActive(i)} className={`h-[2px] transition-all duration-500 ${i===active?'w-10 bg-orange-500':'w-3 bg-neutral-200'}`} />)}</div>
          <button onClick={next} className="w-10 h-10 border border-neutral-200 flex items-center justify-center text-neutral-400 hover:text-orange-500 transition-colors"><CaretRight size={16} /></button>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section ref={ref} className="relative py-24 sm:py-32 overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="relative z-10 max-w-3xl mx-auto px-5 sm:px-8 text-center">
        <h2 className="font-heading text-white leading-[0.92] font-bold uppercase mb-6" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>Get Your Vehicle <span className="text-orange-400">Restored</span></h2>
        <p className="text-white/50 text-sm sm:text-base max-w-xl mx-auto mb-10" style={{ fontFamily: 'var(--font-sans)' }}>Visit us at 40 Craster Rd for a free assessment.</p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link to="/contact" className="group inline-flex items-center justify-center gap-3 bg-orange-500 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-orange-600 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>Book Now <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" /></Link>
          <a href="https://wa.me/263773193931" target="_blank" rel="noopener noreferrer" className="group inline-flex items-center justify-center gap-3 border border-green-500/40 text-green-400 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-green-500/10 transition-all" style={{ fontFamily: 'var(--font-sans)' }}><WhatsappLogo size={18} weight="fill" /> WhatsApp</a>
        </div>
      </motion.div>
    </section>
  );
